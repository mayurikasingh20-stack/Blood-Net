...
function LoginForm({ config, role }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const label = isDonor ? "phone number" : "email";
    ...
    try {
      ...
      if (err.response) {
        ...
        if (status === 401) {
          setError(isDonor ? "Incorrect phone number or password. Please try again." : "Incorrect email or password. Please try again.");
        } else if (status === 404) {
          setError(isDonor ? "This phone number isn't registered. Please sign up first." : "This email isn't registered. Please sign up first.");
        } else {
