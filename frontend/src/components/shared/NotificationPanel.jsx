...
import { Bell, CheckCircle, Droplets, Eye, X } from "lucide-react";
...
export default function NotificationPanel({ notifications = [], onClear, onReadAll }) {
  ...
  return (
    <motion.div className="bg-white rounded-2xl border border-slate-100 shadow-sm" {...fadeUp}>
      <div className="px-4 md:px-6 py-4 border-b border-slate-100 space-y-2">
        <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
          <Bell size={16} className="text-red" />
          Notifications
        </h3>
          <div className="flex items-center gap-3">
            <button onClick={handleReadAll} className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-red transition font-semibold px-2 py-1">
              <Eye size={12} /> Read All
            </button>
            <span className="text-slate-200 text-xs">|</span>
            <button onClick={handleClearAll} className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-red transition font-semibold px-2 py-1">
              <X size={12} /> Clear All
            </button>
          </div>
      </div>
