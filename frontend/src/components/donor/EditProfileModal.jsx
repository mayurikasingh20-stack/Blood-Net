    ...
    <div className="fixed inset-0 bg-ink/50 flex items-center justify-center z-50 px-4">
      <div className="bg-paper rounded-[22px] p-8 w-full max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <form id="edit-profile-form" onSubmit={handleSave} className="space-y-4">
          ...
          <div className="pt-3 border-t border-ink/10">
            ...
            {(form.medications || []).map((m, i) => (
              <div key={i} className="flex items-center justify-between bg-ink/5 rounded-lg px-3 py-2 mb-2">
                ...
                <button type="button" onClick={() => removeMedication(i)}><Trash2 size={14} className="text-red-deep" /></button>
              </div>
            ))}
            <div className="flex gap-2">
              ...
              />
              <button type="button" onClick={addMedication} className="bg-ink text-white px-3 rounded-lg"><Plus size={16} /></button>
            </div>
          </div>
          ...
          <div className="pt-3 border-t border-ink/10">
            ...
            <div className="flex gap-2">
              ...
              />
              <button type="button" onClick={addIllness} className="bg-ink text-white px-3 rounded-lg"><Plus size={16} /></button>
            </div>
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button type="submit" form="edit-profile-form" className="w-2/3 bg-red-deep text-white font-semibold py-3 rounded-full hover:bg-red transition">Save Changes</button>
        </div>
      </div>
    </div>
  );
