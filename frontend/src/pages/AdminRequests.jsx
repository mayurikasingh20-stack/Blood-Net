...
export default function AdminRequests() {
  ...
  async function openDocument(path) {
    try {
      const res = await api.get(`/blood-request/document/${path}`, {
        responseType: "blob",
      });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Track current status of all blood requests across the platform.
          </p>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50">
                ...
                <th className="text-left px-6 py-3 text-xs font-bold text-slate-500 uppercase hidden md:table-cell">
                  City
                </th>
                <th className="text-left px-6 py-3 text-xs font-bold text-slate-500 uppercase hidden md:table-cell">
                  Units
              ...
              {filtered.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50/50 transition">
                  <td className="px-6 py-3">
                    <span className="font-bold text-red">{r.id}</span>
                  </td>
                  <td className="px-6 py-3">
                    <span className="font-bold text-red">{r.blood_group}</span>
                  </td>
                  ...
                  <td className="px-6 py-3">
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${statusStyles[r.status] || "bg-slate-100 text-slate-500"}`}
                    >
                      {r.status
                  ...
                  <td className="px-6 py-3 hidden md:table-cell">
                    {r.request_document ? (
                      <button
                        onClick={() => openDocument(r.request_document)}
