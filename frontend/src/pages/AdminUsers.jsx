...
export default function AdminUsers() {
  ...
  async function handleBlock(user) {
    const reason = window.prompt(
      `Reason for blocking ${user.first_name} ${user.last_name}:`,
      "",
    );
    if (!reason || !reason.trim()) return;
    if (
      !window.confirm(
        `Block ${user.first_name} ${user.last_name} (${user.email || user.phone})? They will be unable to log in.`,
      )
    )
      return;
    setBusyId(user.id);
  ...
  async function handleUnblock(user) {
    const reason = window.prompt(
      `Reason for unlocking ${user.first_name} ${user.last_name}:`,
      "",
    );
    if (!reason || !reason.trim()) return;
    setBusyId(user.id);
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Manage donor, patient, and admin accounts. Blood banks are managed in
          their own section.
        </p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50">
                ...
                <th className="text-left px-4 py-3 text-xs font-bold text-slate-500 uppercase hidden lg:table-cell">
                  City
                </th>
                <th className="text-left px-4 py-3 text-xs font-bold text-slate-500 uppercase hidden md:table-cell">
                  Last Reason
                ...
                return (
                  <tr key={u.id} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className="px-4 py-3 hidden md:table-cell">
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isAdmin ? "bg-violet-50 text-violet-700" : "bg-slate-100 text-slate-600"}`}
                      >
                        {roleLabel(u.roles)}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 hidden md:table-cell max-w-[200px]">
                      {u.last_action ? (
                        <>
                          <span
                            className={`text-[10px] font-bold px-2 py-0.5 rounded-full mr-1.5 ${u.last_action.action === "block" ? "bg-amber-50 text-amber-700" : "bg-emerald-50 text-emerald-600"}`}
                          >
                            {u.last_action.action === "block"
                              ? "Blocked"
                              : "Unlocked"}
                          </span>
                          <span className="text-xs text-slate-500">
                            {u.last_action.reason}
                          </span>
                        </>
                      ) : (
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${u.is_active ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-700"}`}
                      >
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-1.5">
                        ...
                        <button
                          ...
                          title={
                            isAdmin
                              ? "Admin accounts cannot be unblocked"
                              : "Unlock user"
                          }
